...
/* ----------------------------- main ----------------------------- */
export default function NHCDashboard() {
  ...
  if (fetchError === "no-url") return <SetupScreen />;
  if (fetchError === "fetch-failed") return <ErrorScreen />;
...
function SetupScreen() {
  return (
    <div style={{ fontFamily: "Inter, sans-serif", padding: 40, maxWidth: 560 }}>
      <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 12 }}>⚙️ One step to go</div>
      <p style={{ color: "#5A6573", lineHeight: 1.7 }}>
        Open <code>NHCDashboard.jsx</code>, find the line near the top that says<br />
        <code style={{ background: "#F0F2F5", padding: "2px 6px", borderRadius: 4 }}>const APPS_SCRIPT_URL = "YOUR_APPS_SCRIPT_URL_HERE"</code><br />
        and replace it with your deployed Apps Script Web App URL.
      </p>
    </div>
  );
}
...
function ErrorScreen() {
  return (
    <div style={{ fontFamily: "Inter, sans-serif", padding: 40, maxWidth: 560 }}>
      <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 12 }}>⚠️ Could not load data</div>
      <p style={{ color: "#5A6573", lineHeight: 1.7 }}>
        The Apps Script URL returned an error. Make sure:<br />
        1. The deployment is set to <b>Execute as: Me</b> and <b>Access: Anyone</b>.<br />
        2. You copied the <code>/exec</code> URL (not the editor URL).<br />
        3. CORS is not blocked — test by pasting the URL directly in your browser.
      </p>
    </div>
  );
}
...
function LoadingScreen() {
  return (
    <div style={{ fontFamily: "Inter, sans-serif", padding: 40, color: "#5A6573", display: "flex", alignItems: "center", gap: 14 }}>
